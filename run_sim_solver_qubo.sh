...
do
	...
	'-a'|'--annotator')
	    ...
	    ;;
	'--neural-model')
	    neural_args+=(--neural-model "$2")
	    shift 2
	    continue
	    ;;
	'--device')
	    neural_args+=(--device "$2")
	    shift 2
	    continue
	    ;;
	'--neural-beta')
	    neural_args+=(--neural-beta "$2")
	    shift 2
	    continue
	    ;;
	'-c1'|'--const1')
...
if [ "$edge2node" -eq 1 ]; then
    ...
    run_sim_solver_qubo_with_pathfinder_graph.sh "$gfa_filepath" "$query" "$outdir" "$penalties" "$solver" "$num_jobs" "$time_limits" "$annotator" "$const1" "$const2" "${neural_args[@]}"
...
#    ' "$gfa_filepath" "$const1" "$const2")
    ...
    echo "COPY_NUMBERS=$copy_numbers"
        ...
        "$PYTHON" "$QUBO_DIR/build_oriented_qubo_matrix.py" -f "$gfa_filepath" -d "$outdir" -c "$copy_numbers" -p "$penalties"
        "$PYTHON" "$QUBO_DIR/oriented_max_path.py" -s "$solver" -f "$gfa_filepath" -d "$outdir" -j "$num_jobs" -t "$time_limits" -o "$query.gaf" "${neural_args[@]}"
