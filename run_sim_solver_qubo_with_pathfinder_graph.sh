    ...
    ' "$query".subgraph | while IFS= read -r line; do
        ...
        if [[ "$line" == "SUBGRAPH_START:"* ]]; then
            ...
            if [[ -n "$current_nodes_str" ]]; then 
                ...
                "$PYTHON" "$QUBO_DIR/build_oriented_qubo_matrix.py" -f "$output_gfa_file" -d "$outdir" -c "$copy_numbers" -p "$penalties"
                "$PYTHON" "$QUBO_DIR/oriented_max_path.py" -s "$solver" -f "$output_gfa_file" -d "$outdir" -j "$num_jobs" -t "$time_limits" -o "$query.subgraph.$current_subgraph_name.gaf" "${neural_args[@]}"
