...
if [ -e "${SHUF_RANDOM_SOURCE:-/usr/bin/emacs}" ]; then
    ls -1 seq_* | tail -50 | shuf --random-source="${SHUF_RANDOM_SOURCE:-/usr/bin/emacs}" > fofn.all
else
    ls -1 seq_* | tail -50 | shuf > fofn.all
fi
head -"${TRAIN_COUNT:-40}" fofn.all > fofn.train
...
minigraph ${MINIGRAPH_BUILD_OPTS:--l 1000 -d 10000 -n 5,20} ${MINIGRAPH_BUILD_MODE:--xggs} $(cat fofn.train) > pop.gfa 2>pop.minigraph.err
