...
parser.add_argument('--neural-model', help='Checkpoint for neural SeeA*. Required for solver=seea.')
parser.add_argument('--neural-beta', type=float, default=1.0, help='Weight beta in g(n)+beta*h_theta(n).')
parser.add_argument('--device', default='cpu', help='Torch device for neural SeeA* model evaluation.')
...
def main():
    ...
    if args.neural_model:
        os.environ['QPG_SEEA_MODEL'] = args.neural_model
        os.environ['QPG_ACO_MODEL'] = args.neural_model
    ...
    elif qubo_description.solver == Solver.EXACT:
        paths = exact_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.LOCAL:
        paths = local_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.GREEDY_RESIDUAL:
        paths = greedy_residual_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.RANDOM_RESIDUAL:
        paths = random_residual_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.BEAM:
        paths = beam_search_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.ACO:
        paths = aco_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.NEURAL_ACO:
        paths = neural_aco_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.ASTAR:
        paths = astar_sample_qubo(qubo_description)
    elif qubo_description.solver == Solver.SEEA:
        paths = seea_sample_qubo(qubo_description)
    else:
